...
export default function ExpensesPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            // Parallel fetch with a safety timeout and limit for performance
            ...
            const [branchesRes, expensesRes] = await Promise.race([fetchPromise, timeoutPromise]) as any;

            const bData = branchesRes?.data;
            const eData = expensesRes?.data;

            if (bData) {
                const uniqueBranches = bData.filter((branch: any, index: number, self: any[]) =>
            ...
            if (eData) {
                const enriched = eData.map((e: any) => ({
                    ...e,
                    branch_name: bData?.find((b: any) => b.id === e.branch_id)?.name || 'General'
                }));
    ...
    }, [profile?.branch_id, role, fetchData]);
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                onClick={fetchData}
                                disabled={loading}
                                className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm text-slate-400 hover:text-blue-600 hover:shadow-md transition-all active:scale-95 disabled:opacity-50"
                                title="Segarkan Data"
                            >
                                    ...
                                    <div className="w-full max-w-xl max-h-[90vh] bg-white rounded-[2rem] shadow-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
                                        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-8 shrink-0">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <button onClick={() => setShowAddForm(false)} className="p-3 text-slate-400 hover:text-white">
                                                    <AlertCircle size={20} />
                                                </button>
                                            </div>
                                        </div>
                                            ...
                                            <form id="expense-form" onSubmit={handleAddExpense} className="space-y-5">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                    <div className="space-y-2">
                                                        ...
                                                        <select
                                                            className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-5 text-sm font-bold text-slate-700"
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <textarea
                                                        className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-5 text-sm font-medium text-slate-700 min-h-[100px]"
                                        ...
                                        <div className="p-6 bg-slate-50 border-t border-slate-100 shrink-0">
                                            <div className="flex gap-4">
                                                <button type="button" onClick={() => setShowAddForm(false)} className="flex-1 py-4 px-6 rounded-2xl bg-white border border-slate-200 text-slate-500 font-black text-xs uppercase tracking-widest">Batal</button>
                                                <button type="submit" form="expense-form" disabled={submitting} className="flex-[2] py-4 px-6 rounded-2xl bg-slate-900 text-white font-black text-xs uppercase tracking-widest">
