...
export default function BookingOnlinePage() {
    ...
    const handleBooking = async () => {
        ...
        try {
            // Generate simple code: BK-MBR-RANDOM
            ...
            const { error } = await supabase.from("bookings").insert([{
                ...
                mitra_id: profile.id, // Using mitra_id as member link for now
                branch_id: selectedBranch.id,
                booking_code: code,
                booking_type: 'referral', // Existing type for member-linked bookings
                status: 'pending'
            }]);
                            ...
                            >
                                <Card className="border-slate-100">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-sm font-bold text-slate-700">Data Kendaraan</label>
                                            <div className="space-y-4">
                                                <div className="relative">
                                                    <Car className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                                    <input 
                                                        type="text" 
                                                        placeholder="Merek & Tipe Mobil (e.g. Avanza)"
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input 
                                                        type="text" 
                                                        placeholder="Nomor Polisi (e.g. B 1234 ABC)"
                                    ...
                                    <div className="mt-10 flex justify-end">
                                        <Button 
                                            disabled={!selectedBranch || !carModel || !licensePlate}
                                            onClick={() => setStep(2)}
                                            className="h-14 px-8 rounded-2xl shadow-xl shadow-primary/20 gap-2 font-black"
                                        >
                                            LANJUT PILIH JADWAL
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-4">
                                            ...
                                            <input 
                                                ...
                                                className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:border-primary focus:bg-white transition-all font-bold"
                                                value={serviceDate}
                                                onChange={(e) => setServiceDate(e.target.value)}
                                            />
                                            <p className="text-xs text-slate-500 bg-blue-50 p-3 rounded-xl border border-blue-100 flex gap-2">
                                                <AlertCircle size={14} className="shrink-0 text-blue-500" />
                                                Pemesanan minimal dilakukan 1 hari sebelum kedatangan.
                                            </p>
                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex gap-3">
                                            ...
                                            <p className="text-xs text-amber-700 font-medium leading-relaxed">
                                                Dengan mengonfirmasi, Anda bersedia datang tepat waktu. Jika berhalangan hadir, mohon batalkan 3 jam sebelum jadwal.
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex gap-4 pt-6">
                                            ...
                                            <Button 
                                                ...
                                                className="flex-[2] h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl shadow-xl shadow-emerald-500/20 gap-2 font-black"
                                            >
                            ...
                            >
                                ...
                                <div className="bg-white border-4 border-dashed border-slate-100 p-8 rounded-[2rem] max-w-sm mx-auto shadow-2xl relative overflow-hidden">
                                    ...
                                    <div className="mt-8 pt-8 border-t border-slate-50 grid grid-cols-2 gap-4 text-left">
                                        <div>
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Cabang</p>
                                            <p className="text-xs font-bold text-slate-600">{selectedBranch?.name}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Waktu</p>
                                            <p className="text-xs font-bold text-slate-600">{serviceTime}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    <Button onClick={() => window.print()} variant="outline" className="h-14 px-8 rounded-2xl font-bold">
                                        CETAK / SIMPAN PDF
                                    </Button>
