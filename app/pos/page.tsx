...
function POSContent() {
    ...
    useEffect(() => {
        const searchBookings = async () => {
            ...
            // Mengambil 500 data antrian terbaru (termasuk pending, processing, dan draft/completed baru-baru ini)
            const { data } = await supabase
                .from("bookings")
            ...
            if (data) {
                // Hilangkan semua spasi dan simbol, ubah ke lowercase
                const cleanInput = bookingSearchTerm.replace(/[^a-z0-9]/gi, '').toLowerCase();

                const filteredResults = data.filter(b => {
                    // Semua booking, baik referral maupun direct, dicari agar bisa di convert nanti

                    // Bersihkan setiap data dari database untuk dicocokkan
                    const plate = (b.license_plate || "").replace(/[^a-z0-9]/gi, '').toLowerCase();
                    const name = (b.customer_name || "").replace(/[^a-z0-9]/gi, '').toLowerCase();
                    const car = (b.car_model || "").replace(/[^a-z0-9]/gi, '').toLowerCase();
                    const code = (b.booking_code || "").replace(/[^a-z0-9]/gi, '').toLowerCase();

                    // Pencarian pintar: cocokkan jika input ada di plat, nama, tipe mobil, atau kode booking
                    return plate.includes(cleanInput) ||
                        name.includes(cleanInput) ||
                        car.includes(cleanInput) ||
                        code.includes(cleanInput);
                }).slice(0, 8); // Tampilkan sampai 8 pilihan prediktif

                setBookingResults(filteredResults);
            }
            setIsSearchingBooking(false);
        };
        const timer = setTimeout(searchBookings, 300);
                    ...
                    {bookingData && (
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-4 sm:p-5">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                                ...
                                {bookingData.mitra && (
                                    <div className="bg-white/70 rounded-xl p-3">
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">Mitra</p>
                                        <p className="font-bold text-emerald-700 truncate">{bookingData.mitra.full_name}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    if (b.booking_type === 'referral' || b.mitra_id) {
                                                                        setConversionPrompt({
                                                                            show: true,
                                                                            booking: b,
                                                                            title: "History Referral Mitra",
                                                                            message: "Pelanggan ini sebelumnya masuk via Mitra. Harap daftar ulang untuk kedatangan kali ini."
                                                                        });
                                                                ...
                                                                <div className="flex items-center justify-between mb-2">
                                                                    ...
                                                                    <div className="flex items-center gap-1.5">
                                                                        ...
                                                                        <Badge variant={b.booking_type === 'direct' ? 'neutral' : 'info'} className="text-[9px]">
                                                                            {b.booking_type === 'direct' ? 'DIRECT' : 'MITRA'}
                                                                        </Badge>
                                                                    </div>
                                                                </div>
